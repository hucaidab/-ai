...
| V6 | 已完成（交互层 P0） | **对话式设计助手**：① `analyzeNeed` 补全引导（严重缺角色/步骤 → 结构化引导问题反问，模板命中 ≤25 字直出不被拦截；命令行 exit 3 / 网页 needMore 弹引导 / WorkBuddy 转述）；② `agent-flow --edit "描述"` 自动接续最近一版 req.json（多轮对话改图闭环，交付提示下一轮命令）；③ `renderOne` 验收失败自动修复循环（≤3 轮：失败项→autoFix 规则修复/泳道降级提示→重渲染验证，输出修复说明）。**修真 bug：parse-flow 节点 id `END` 被 `startsWith('end')` 误判为 subgraph 结束关键字 → 精确匹配独立 `end` 行**（单测回归锁定）。单测 14/14、模板 100/100 |
| V7 | 已完成（P1 建模质量 + P2 工程健壮性） | **P1**：`repairSchema` 强化（判断节点 action 自动补`?`、判断出口标签归一"是/否/同意/拒绝"→"通过/驳回"、role 去"人员/岗位"冗余后缀）；SYSTEM prompt few-shot 扩为双示例（单部门+跨部门泳道，规则新增 lanes 按流程顺序/驳回闭环指向）；真实 LLM 验证：跨部门报销 3 泳道顺序正确、双判断带标签、逆向标记全对。**P2**：agent-flow 全局 uncaughtException/unhandledRejection 友好兜底（不再裸崩堆栈）；性能压测 1000 节点 parse+layout+render 共 32ms（线性扩展，无 O(n²)）；CI 脚本 `.github/workflows/ci.yml`（push/PR → 单测 15 项 + 100 模板验收 + 匹配/渲染冒烟）。单测 15/15 |
