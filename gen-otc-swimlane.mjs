// ============================================================
// ERP 销售订单（Order-to-Cash, OTC）部门 + 岗位两级泳道图 - 自研布局器
// 8 岗位：客户 | 销售员 | 销售经理 | 销售总监 | 信用专员 | 仓管员 | 应收会计 | 出纳
// 销售部内嵌第三级审批链：销售经理 → 销售总监（大额会签）
// 实线=正向流转，虚线=逆向/异常
// ============================================================
...
// ---------- 部门与岗位 ----------
const DEPTS = [
  { id: 'cus', name: '客户', sub: 'Customer',      color: '#6e7781', tint: '#f6f8fa', label: '#424a53',
    roles: [ { id: 'cus', name: '客户' } ] },
  { id: 'sal', name: '销售部', sub: 'Sales',        color: '#0969da', tint: '#ddf4ff', label: '#0a3069',
    roles: [ { id: 'sal_rep', name: '销售员' }, { id: 'sal_mgr', name: '销售经理' }, { id: 'sal_dir', name: '销售总监·大额会签' } ] },
  { id: 'crd', name: '信用部', sub: 'Credit',       color: '#0f766e', tint: '#e6f7f5', label: '#115e59',
    roles: [ { id: 'crd_ofc', name: '信用专员' } ] },
  { id: 'wh',  name: '仓储部', sub: 'Warehouse',    color: '#1a7f37', tint: '#dafbe1', label: '#0a3d1e',
    roles: [ { id: 'wh_kpr', name: '仓管员' } ] },
  { id: 'fin', name: '财务部', sub: 'Finance',      color: '#8250df', tint: '#fbefff', label: '#3e1f6f',
    roles: [ { id: 'fin_ar', name: '应收会计' }, { id: 'fin_csh', name: '出纳' } ] },
]
...
// ---------- 节点 ----------
const ROWS = [
  { nodes: [{ id: 'C0', dept: 'cus', role: 'cus', shape: 'ellipse', lines: ['开始', '发起订单需求'] }] },
  { nodes: [{ id: 'S1', dept: 'sal', role: 'sal_rep', shape: 'rect', lines: ['创建销售订单 SO'] }] },
  { nodes: [{ id: 'S2', dept: 'sal', role: 'sal_mgr', shape: 'rect', lines: ['审核订单'] }] },
  { nodes: [{ id: 'S3', dept: 'sal', role: 'sal_mgr', shape: 'diamond', lines: ['金额超限?'] }] },
  { nodes: [{ id: 'S4', dept: 'sal', role: 'sal_dir', shape: 'rect', lines: ['总监会签'] }] },
  { nodes: [{ id: 'K1', dept: 'crd', role: 'crd_ofc', shape: 'rect', lines: ['信用审核'] }] },
  { nodes: [{ id: 'S5', dept: 'sal', role: 'sal_rep', shape: 'rect', lines: ['确认订单并发送'] }] },
  { nodes: [{ id: 'C1', dept: 'cus', role: 'cus', shape: 'rect', lines: ['确认订单'] }] },
  { nodes: [{ id: 'W1', dept: 'wh', role: 'wh_kpr', shape: 'rect', lines: ['拣货出库'] }] },
  { nodes: [{ id: 'W2', dept: 'wh', role: 'wh_kpr', shape: 'rect', lines: ['物流发货'] }] },
  { nodes: [{ id: 'C2', dept: 'cus', role: 'cus', shape: 'rect', lines: ['收货并验收'] }] },
  { nodes: [{ id: 'F1', dept: 'fin', role: 'fin_ar', shape: 'rect', lines: ['开具发票'] }] },
  { nodes: [{ id: 'F2', dept: 'fin', role: 'fin_ar', shape: 'rect', lines: ['应收对账'] }] },
  { nodes: [{ id: 'C3', dept: 'cus', role: 'cus', shape: 'rect', lines: ['支付货款'] }] },
  { nodes: [{ id: 'F3', dept: 'fin', role: 'fin_csh', shape: 'rect', lines: ['收款并核销'] }] },
  { nodes: [{ id: 'F4', dept: 'fin', role: 'fin_csh', shape: 'ellipse', lines: ['结束', '订单完成'] }] },
]
...
const EDGES = [
  // 正向主链
  { f: 'C0', t: 'S1', label: '询单 / 下单' },
  { f: 'S1', t: 'S2', label: '提交审核' },
  { f: 'S2', t: 'S3', label: '' },
  // 销售部内嵌第三级：经理 → 总监会签（大额）
  { f: 'S3', t: 'S4', label: '超限' },
  { f: 'S4', t: 'K1', label: '会签通过' },
  { f: 'S3', t: 'K1', label: '未超限' },
  { f: 'K1', t: 'S5', label: '信用通过' },
  { f: 'S5', t: 'C1', label: '发送订单' },
  { f: 'C1', t: 'W1', label: '确认' },
  { f: 'W1', t: 'W2', label: '' },
  { f: 'W2', t: 'C2', label: '发货' },
  { f: 'C2', t: 'F1', label: '验收完成' },
  { f: 'F1', t: 'F2', label: '' },
  { f: 'F2', t: 'C3', label: '对账单' },
  { f: 'C3', t: 'F3', label: '付款' },
  { f: 'F3', t: 'F4', label: '' },
  // 逆向 / 异常
  { f: 'S2', t: 'S1', label: '驳回·修改重提', dash: true },
  { f: 'S4', t: 'S1', label: '会签不通过', dash: true },
  { f: 'K1', t: 'S1', label: '信用不通过·补充', dash: true },
  { f: 'C1', t: 'S1', label: '取消订单', dash: true },
  { f: 'C2', t: 'W2', label: '退货', dash: true },
  { f: 'F2', t: 'C3', label: '对账差异·催收', dash: true },
  { f: 'F3', t: 'C3', label: '逾期催收', dash: true },
]
...
svg += `<text x="${MARGIN}" y="22" font-size="15" font-weight="700" fill="#24292f">ERP 销售订单（Order-to-Cash）· 部门 + 岗位两级泳道图</text>`
svg += `<text x="${MARGIN + 400}" y="22" font-size="10.5" fill="#57606a">实线=正向 · 虚线=逆向/异常 · 销售部内嵌第三级审批链（经理→总监大额会签）· 8 岗位</text>`

svg += '</svg>'

fs.writeFileSync('C:/Users/1/WorkBuddy/2026-08-12-15-26-19/diagram-otc-swimlane.svg', svg, 'utf-8')
console.log('✅ OTC 泳道图生成: diagram-otc-swimlane.svg')
